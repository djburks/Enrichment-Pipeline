...
# Load Custom annotations via geneID2GO.

	geneID2GO <-readMappings("Hassleriana_GO.map")
        
# Make the TopGOdata Object

	GOdata <- new("topGOdata",ontology="BP",allGenes=geneList,annot=annFUN.gene2GO,gene2GO=geneID2GO)
	GOdata2 <- new("topGOdata",ontology="MF",allGenes=geneList,annot=annFUN.gene2GO,gene2GO=geneID2GO)
	GOdata3 <- new("topGOdata",ontology="CC",allGenes=geneList,annot=annFUN.gene2GO,gene2GO=geneID2GO)
